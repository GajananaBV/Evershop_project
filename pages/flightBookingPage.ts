...
export class FlightBookingPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.accountList = this.loginModal.locator(
      'ul[data-cy="LoginFlowPopup_82"] li'
    );
  ...
  async selectAccount(accountType: string) {
    ...
    const normalized = accountType.toLowerCase().includes("personal")
      ? "personal"
      : "myBiz";
    await modal
      .locator(
        `ul[data-cy="LoginFlowPopup_82"] li[data-acctype="${normalized}"]`
      )
      .click();
