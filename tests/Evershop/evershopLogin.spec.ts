...
test.describe("Subscription Flow", () => {
  test("User signup and Admin validation", async ({ page }) => {
    ...
    await test.step("Navigate to homepage and open signup form", async () => {
      ...
      await expect(
        getLocator(page, applocators.linkCreateAccount)
      ).toBeEnabled();
    ...
    await test.step("Fill signup form and register", async () => {
      ...
      await getLocator(page, applocators.fullNameInput).fill(
        testData.user.name
      );
      await getLocator(page, applocators.emailInput).fill(randomEmail);
      await getLocator(page, applocators.passwordInput).fill(
    ...
    await test.step("Login as Admin", async () => {
      ...
      await expect(
        getLocator(page, applocators.customersLink)
      ).toBeVisible();
